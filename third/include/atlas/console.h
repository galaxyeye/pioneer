...
namespace clown {
  ...
  static const char* DEFAULT_HISTORY_PATH = "/tmp/atlas_history_console";
  ...
  public:
    ...
    console(const char* prompt, const char* history_path = DEFAULT_HISTORY_PATH) :
        _rawmode(0), _history_len(0), _history_index(0), _history(nullptr), completion_callback(
            nullptr), _prompt(prompt), _history_path(history_path) {
  ...
  protected:
    ...
    void refresh_line(int fd, const char *prompt, char *buf, size_t len, size_t pos, size_t cols);
  ...
  int console::prompt_cmd_line(int fd, char *buf, size_t buflen, const char *prompt) {
    ...
    while (1) {
      ...
      if (c == 9 && completion_callback != nullptr) { /* tab */
