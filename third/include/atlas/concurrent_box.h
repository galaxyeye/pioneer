...
namespace atlas {
  ...
  public:
    ...
    concurrent_box(std::chrono::microseconds time) : _wait_time(time) {}
...
//      iterator it = _connections.end();
//
//      auto f = [this, &conn, &it, &ip_port]() -> bool {
//        if (ip_port.find(":") != std::string::npos) {
//          std::tie(conn, it) = this->get_by_ip_port(ip_port);
//        }
//
//        std::tie(conn, it) = this->get_by_ip(ip_port);
//
//        return conn.get() != nullptr;
//      };
//
//      // wait until there connection for ip_port exists
//      std::unique_lock<std::mutex> lock(_mutex);
//      if (!_found.wait_for(lock, _wait_time, f)) {
...
//
//      DLOG(INFO) << "take " << conn->peerAddress().toIpPort() << ", pool size : " << _connections.size();
//
//      return conn;
    }
...
//      std::unique_lock<std::mutex> lock(_mutex);
//
//      // wait for there is at least one connections in the pool
//      if (!_found.wait_for(lock, _wait_time, [this]() { return !_connections.empty(); })) {
//        throw std::system_error(std::make_error_code(std::errc::timed_out));
//      }
//
//      mn::TcpConnectionPtr conn;
//      size_t step = get_random_index();
//
//      DLOG(INFO) << "take the " << step << "th connection";
//
...
//      }
//      else {
//        conn = _connections.begin()->second;
